...
const cardStyle: React.CSSProperties = {
  padding: '10px 12px', backgroundColor: 'var(--cf-surface)',
  borderRadius: 'var(--cf-radius-lg)', display: 'flex',
  flexDirection: 'column', gap: 8, animation: 'fadeIn 0.25s ease',
};
...
const labelStyle: React.CSSProperties = {
  color: 'var(--cf-text-dim)', fontSize: 10, fontWeight: 500,
  textTransform: 'uppercase', letterSpacing: 0.5,
};
const valueStyle: React.CSSProperties = { fontWeight: 700, fontSize: 15 };
const progressBgStyle: React.CSSProperties = {
  width: '100%', height: 4, backgroundColor: 'var(--cf-border)',
  borderRadius: 2, overflow: 'hidden',
};
const progressFillStyle: React.CSSProperties = {
  height: '100%', backgroundColor: 'var(--cf-success)',
  borderRadius: 2, transition: 'width 0.4s ease',
};

/** Shimmer skeleton bar — used during loading. */
const skeletonBar: React.CSSProperties = {
  backgroundColor: 'var(--cf-border)', borderRadius: 4,
  background: 'linear-gradient(90deg, var(--cf-border) 25%, var(--cf-surface-hover) 50%, var(--cf-border) 75%)',
