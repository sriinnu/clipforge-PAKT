...
import type {
  DocumentNode, BodyNode, ScalarNode, TabularArrayNode,
  InlineArrayNode, ListArrayNode, KeyValueNode, ObjectNode,
  TabularRowNode, ListItemNode, HeaderNode, StringScalar, SourcePosition,
...
const obj = (key: string, ch: BodyNode[]): ObjectNode =>
  ({ type: 'object', key, children: ch, position: p });
const row = (vals: ScalarNode[]): TabularRowNode =>
  ({ type: 'tabularRow', values: vals, position: p });
const item = (ch: BodyNode[]): ListItemNode =>
  ({ type: 'listItem', children: ch, position: p });
  ...
  for (const nd of nodes) {
    switch (nd.type) {
      case 'keyValue':
        ...
        break;
      case 'object': result.push(...collectValues(nd.children)); break;
      case 'tabularArray':
...
describe('L2: high repetition', () => {
  it('creates dictionary entries for repeated values', () => {
    ...
    const tab: TabularArrayNode = {
      type: 'tabularArray', key: 'employees', count: 6,
      fields: ['id', 'name', 'dept', 'status'], position: p,
      rows: [
...
describe('L2: no repetition', () => {
  it('passes through when all values are unique', () => {
    const input = doc([kv('a', s('Alice')), kv('b', s('developer')),
      kv('c', s('Hyderabad')), kv('d', s('TypeScript'))]);
...
describe('L2: below threshold', () => {
  it('does not alias values appearing only 2 times', () => {
    const input = doc([kv('d1', s('Engineering')), kv('d2', s('Engineering')),
      kv('s1', s('active')), kv('s2', s('active'))]);
    const compressed = compressL2(input);
    if (compressed.dictionary !== null) expect(compressed.dictionary.entries).toHaveLength(0);
    const vals = collectValues(compressed.body);
...
describe('L2: roundtrip', () => {
  it('compressL2 -> decompressL2 yields identical scalar values', () => {
    const tab: TabularArrayNode = {
      type: 'tabularArray', key: 'projects', count: 5,
      fields: ['id', 'name', 'dept', 'status'], position: p,
      rows: [
    ...
    const input = doc([kv('org', s('YugenLab')), tab, kv('tags', s('Engineering'))],
      [hdr('from', 'json')]);
  ...
  it('preserves headers through compress/decompress', () => {
    const input = doc([kv('d1', s('Engineering')), kv('d2', s('Engineering')),
      kv('d3', s('Engineering'))], [hdr('from', 'json'), hdr('version', '1.0.0')]);
    const decompressed = decompressL2(compressL2(input));
...
describe('L2: literal dollar values', () => {
  it('does not alias quoted string values starting with $', () => {
    ...
    const input = doc([kv('alias', s('$a', true)),
      kv('d1', s('Engineering')), kv('d2', s('Engineering')),
      kv('d3', s('Engineering')), kv('d4', s('Engineering')),
      kv('d5', s('Engineering')), kv('d6', s('Engineering'))]);
    const compressed = compressL2(input);
  ...
  it('expands both quoted and unquoted $-aliases during decompression', () => {
    ...
    const input: DocumentNode = {
      type: 'document', headers: [], position: p,
      dictionary: { type: 'dictBlock', position: p,
...
describe('L2: mixed node types', () => {
  it('scans and aliases values across all node types', () => {
    ...
    const inline: InlineArrayNode = {
      type: 'inlineArray', key: 'tags', count: 1, position: p,
      values: [s('Engineering')],
    };
    const list: ListArrayNode = {
      type: 'listArray', key: 'items', count: 1, position: p,
...
describe('L2: alias ordering', () => {
  it('assigns $a to the highest net-savings value', () => {
    ...
    const tab: TabularArrayNode = {
      type: 'tabularArray', key: 'data', count: 8,
      fields: ['dept', 'status'], position: p,
      rows: [
...
describe('L2: preserves structure', () => {
  it('preserves headers, object nesting, and array types', () => {
    const nested = obj('config', [kv('d', s('Engineering')),
      obj('inner', [kv('d', s('Engineering')), kv('d2', s('Engineering'))])]);
    const tab: TabularArrayNode = {
      type: 'tabularArray', key: 'items', count: 2,
      fields: ['id', 'val'], position: p,
      rows: [row([n(1), s('Engineering')]), row([n(2), s('Engineering')])],
    };
  ...
  it('does not mutate the input document', () => {
    const input = doc([kv('a', s('Engineering')), kv('b', s('Engineering')),
      kv('c', s('Engineering'))]);
...
describe('extractDictEntries', () => {
  ...
  it('returns DictEntry[] with occurrences and tokensSaved', () => {
    ...
    const tab: TabularArrayNode = {
      type: 'tabularArray', key: 'data', count: 6, fields: ['dept'], position: p,
...
describe('L2: edge cases', () => {
  ...
  it('does not alias short values (1 char)', () => {
    const input = doc([kv('a1', s('x')), kv('a2', s('x')),
      kv('a3', s('x')), kv('a4', s('x'))]);
    const compressed = compressL2(input);
    if (compressed.dictionary !== null) expect(compressed.dictionary.entries).toHaveLength(0);
  });
  ...
  it('does not alias non-string scalars', () => {
    const input = doc([kv('a1', n(42)), kv('a2', n(42)), kv('a3', n(42)),
      kv('b1', b(true)), kv('b2', b(true)), kv('b3', b(true)),
      kv('c1', nil()), kv('c2', nil()), kv('c3', nil())]);
    const compressed = compressL2(input);
    if (compressed.dictionary !== null) expect(compressed.dictionary.entries).toHaveLength(0);
  });
});
