...
import { mergeExternalDict } from './dict-external.js';
...
export { cmdReport } from './cli-commands-report.js';
 ...
 *
 * With `--dict-placement system`, the `@dict` block is written to the
 * `--dict-out` file (required in that mode) and the dict-free body goes
 * to stdout — see `cli-commands-dict.ts`.
 *
 ...
 *
 * Reads PAKT input and writes the decompressed text to stdout. With
 * `--dict <file-or-string>`, an external dictionary block (produced by
 * `compress --dict-placement system`) is merged before alias expansion.
 *
 * @param args - Parsed CLI arguments (file, --to / --dict flags)
 * @param readInput - Function that resolves a file path or stdin to a string
 */
export function cmdDecompress(
  ...
  const dict = resolveExternalDict(args);

  /* Validate the merged document — a dict-free body legitimately uses
     aliases its external dictionary defines, so validating the raw input
     alone would report undefined aliases. */
  assertValidPaktInput(dict !== undefined ? mergeExternalDict(input, dict) : input);

  const decompressOptions: import('./types.js').DecompressOptions = {};
  if (outputFormat) decompressOptions.to = outputFormat;
  if (dict !== undefined) decompressOptions.dict = dict;
  const result = decompress(input, decompressOptions);
