...
console.log(`  After L1 (struct) : ${l1Only.compressedTokens} tokens (${l1Only.savings.totalPercent}% saved)`);
console.log(`  After L1+L2 (dict): ${l1l2.compressedTokens} tokens (${l1l2.savings.totalPercent}% saved)`);
...
if (l1l2.dictionary.length > 0) {
  ...
  for (const e of l1l2.dictionary.slice(0, 5)) {
    console.log(`    ${e.alias} -> "${e.expansion}" (${e.occurrences}x, ${e.tokensSaved} tokens saved)`);
  }
}
console.log('');
...
describe('layers — L1 only vs L1+L2 (nested config)', () => {
  ...
  console.log(`  After L1 (struct) : ${ncL1.compressedTokens} tokens (${ncL1.savings.totalPercent}% saved)`);
  console.log(`  After L1+L2 (dict): ${ncL1L2.compressedTokens} tokens (${ncL1L2.savings.totalPercent}% saved)`);
